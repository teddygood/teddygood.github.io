...
OpenBLAS는 BLAS(Basic Linear Algebra Subprograms)와 일부 LAPACK 연산을 빠르게 실행하기 위한 고성능 선형대수 라이브러리다. 이름 그대로 GotoBLAS 계열 구현을 기반으로 발전해 왔고, CPU 아키텍처별 최적화 커널을 많이 포함한다.
...
이 중에서 기여할 때 가장 자주 보는 것은 `KERNEL` 설정과 연산별 커널 파일이다.
...
## OpenBLAS 기여에서 자주 만나는 연산

wasm 쪽을 포함해 OpenBLAS에 기여할 때 자주 보게 되는 대표 연산은 다음과 같다.
...
- [SIMD](SIMD.md)
- [WebAssembly SIMD](WebAssembly%20SIMD.md)
- [WASM128_GENERIC](WASM128_GENERIC.md)
- [GEMM](GEMM.md)
- [TRSM](TRSM.md)
- [AXPY](AXPY.md)
