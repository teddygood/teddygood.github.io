...
Pyodide는 Python runtime과 과학 계산 패키지를 WebAssembly 위에서 실행하게 해주는 프로젝트다. 브라우저 안에서 NumPy, SciPy 같은 패키지를 돌릴 수 있다는 점이 흥미로웠고, WebAssembly를 공부하면서 실제 패키지 빌드와 성능 검증이 어떻게 이어지는지 직접 확인해보고 싶었다.

처음에는 작은 빌드 도구 개선에서 시작했다. 이후 WebAssembly SIMD가 Pyodide 환경에서 실제로 동작하는지 확인하는 이슈를 보면서, 단순히 flag를 켜는 것이 아니라 테스트 패키지, OpenBLAS 빌드, 벤치마크까지 이어지는 검증 흐름을 만들어보고 싶었다.
...
빌드 시스템 개선에서 먼저 정리해야 했던 문제는 "무엇을 지울 것인가"였다. 소스와 중간 빌드 산출물은 다시 만들 수 있지만, 이미 만들어 둔 wheel이 들어 있는 `dist`까지 지우면 사용자가 의도하지 않은 결과를 잃을 수 있다. 그래서 정리 로직은 빌드 작업 공간을 비우되 `dist`는 기본적으로 보존하는 방향으로 잡았고, 이후 별도 CLI 명령으로 꺼내 쓸 수 있게 나눴다.

SIMD 쪽에서는 compile flag만 보고 성공이라고 말할 수 없었다. Emscripten이 `wasm_simd128.h`와 SSE/AVX intrinsic을 WebAssembly SIMD로 어떻게 바꾸는지 실제 wasm test로 확인해야 했다. 그래서 `test-simd` 패키지를 먼저 만들고, 직접 SIMD intrinsic을 쓰는 경로와 x86 intrinsic이 변환되는 경로를 분리해 검증했다.

OpenBLAS 적용에서는 "SIMD를 켰다"보다 "NumPy/SciPy 하위 BLAS 연산에서 차이를 볼 수 있는가"가 중요했다. 기존 빌드가 SIMD를 충분히 활용하지 못한다는 점을 이슈로 정리하고, `libopenblas-simd` 레시피와 `test-openblas-simd`를 추가했다. `sdot`, `sgemm` 같은 연산을 기준으로 비교할 수 있게 만들어서, 기능 추가와 성능 확인을 같은 PR 안에서 설명할 수 있었다.
