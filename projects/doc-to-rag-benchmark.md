...
이 프로젝트를 하며 가장 크게 깨달은 건 분명했다. RAG 성능을 안정적으로 끌어올리는 데에는 "좋은 모델 하나"보다 "신뢰할 수 있는 평가 데이터와 선택 기준"이 더 중요했다.

내 역할 기준으로도, 파인튜닝 성능 자체를 올리는 것보다 실험 루프를 고정하고 결과를 재현 가능한 형태로 남긴 일이 이후 개선 속도를 더 크게 끌어올렸다.
